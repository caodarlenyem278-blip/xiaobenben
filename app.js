...
function renderJournals() {
    ...
    if (data.journals.length === 0) {
        el.innerHTML = '<div class="empty-state"><div class="empty-icon">??</div><p>还没有记录，写下今天的故事吧</p></div>';
        return;
    }
        ...
        <div class="entry-card">
            ...
            <div class="entry-meta">
                ...
                <div class="entry-actions">
                    <button class="entry-btn delete" onclick="deleteJournal(${i})">???</button>
                </div>
            </div>
        </div>
    `).join('');
...
document.getElementById('journalBtn').addEventListener('click', () => {
    ...
    if (!text) { showToast('写点内容再保存吧 ??'); return; }
...
function renderTodos() {
    ...
    if (data.todos.length === 0) {
        el.innerHTML = '<div class="empty-state"><div class="empty-icon">??</div><p>还没有待办事项，添加一个吧</p></div>';
        return;
    }
    el.innerHTML = data.todos.map((t, i) => `
        <div class="entry-card todo-card">
            <div class="todo-top">
                ...
                <button class="entry-btn delete" onclick="deleteTodo(${i})">???</button>
            </div>
        </div>
    `).join('');
}
function renderDones() {
    ...
    if (data.dones.length === 0) {
        el.innerHTML = '<div class="empty-state"><div class="empty-icon">??</div><p>还没有完成的事项，加油！</p></div>';
        return;
    }
    el.innerHTML = data.dones.map((d, i) => `
        <div class="entry-card done-card">
            <div class="todo-top">
                ...
                <button class="entry-btn delete" onclick="deleteDone(${i})">???</button>
            </div>
        </div>
    `).join('');
}
...
document.getElementById('todoBtn').addEventListener('click', () => {
    ...
    const text = input.value.trim();
    if (!text) { showToast('输入要做什么吧 ??'); return; }
    data.todos.unshift({
        text,
        time: new Date().toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })
    });
...
document.querySelectorAll('.tab').forEach(function(tab) {
    if (tab.dataset.tab === 'diet') tab.addEventListener('click', function() { renderCalendar(); });
});
